...
- OS access only inside the platform directories (`engine/platform/unix`, `engine/platform/win32`, `engine/platform/sdl`) and the
...
- Bugs found while doing something else go in `docs/bugs.md` (moves to `docs/bugs.md`
...
- `engine/qcommon` shared core: cvars, commands, filesystem, network, collision (cm_*), VM
...
- `engine/renderercommon`, `engine/renderer` (OpenGL1, legacy), `engine/renderervk` (Vulkan, reference
  renderer); `engine/renderer2` is not built and will be deleted
...
- `game/cgame`, `game/game`, `game/ui` shared ABI headers today; native game code lands here in #2
