...
==================
*/
void BotDrawDebugPolygons( void ( *drawPoly )( int color, int numPoints, float *points ), int value [[maybe_unused]] ) {
...
static void BotImport_DebugPolygonShow( int id, int color, int numPoints, vec3_t *points ) {
	...
	if ( (unsigned)id >= (unsigned int)bot_maxdebugpolys )
		return;
...
static void BotClientCommand( int client, const char *command ) {
	if ( (unsigned)client < (unsigned int)sv.maxclients ) {
...
void SV_BotInitBotLib( void ) {
	...
	Q_ASSERT(botlib_export); // somehow we end up with a zero import.
}
...
int SV_BotGetSnapshotEntity( int client, int sequence ) {
	if ( (unsigned)client < (unsigned int)sv.maxclients ) {
		...
		if ( (unsigned)sequence >= (unsigned int)frame->num_entities ) {
			return -1;
		}
