...
static qboolean CM_PlaneEqual( const patchPlane_t *p, const float plane[4], int *flipped ) {
	...
	if (
		fabs( (double)( p->plane[0] - plane[0] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[1] - plane[1] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[2] - plane[2] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[3] - plane[3] ) ) < DIST_EPSILON ) {
		*flipped = qfalse;
		return qtrue;
	}
	...
	if (
		fabs( (double)( p->plane[0] - invplane[0] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[1] - invplane[1] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[2] - invplane[2] ) ) < NORMAL_EPSILON && fabs( (double)( p->plane[3] - invplane[3] ) ) < DIST_EPSILON ) {
		*flipped = qtrue;
		return qtrue;
	}
