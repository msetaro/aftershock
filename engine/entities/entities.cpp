...
static bool ValidField( const entityDefinitionField_t &field, uint32_t mask ) {
	...
	static const rule_t rules[] = {
		...
		{ "replication", "rep_priority", 8, 1, 0, 3 }, { "replication", "rep_radius", 8, 2, 0, 32768 },
	...
	for ( uint32_t i = 0; i < header.count; ++i ) {
		...
		for ( uint32_t j = first; j < first + definition.fieldCount; ++j ) {
			...
			if ( !ValidField( field, definition.components ) )
				return false;
			if ( !strcmp( field.key, "rep_priority" ) ) {
				uint32_t priority = 0;
				std::from_chars( field.value, field.value + strlen( field.value ), priority );
...
bool Entity_SetField( entityDefinitions_t *definitions, const char *name, const char *key, const char *value ) {
	if ( !definitions || !name || !key || !value || strlen( value ) >= sizeof( entityDefinitionField_t::value ) )
		return false;
	const auto *definition = Entity_FindDefinition( *definitions, name );
	const auto *field = definition ? Entity_Field( *definitions, *definition, key ) : nullptr;
	if ( !field )
		return false;
	auto replacement = *field;
	memset( replacement.value, 0, sizeof( replacement.value ) );
	memcpy( replacement.value, value, strlen( value ) );
	if ( !ValidField( replacement, definition->components ) )
		return false;
	auto &edited = definitions->definitions[definition - definitions->definitions];
...
size_t Entity_WriteDefinitions( const entityDefinitions_t &definitions, void *data, size_t capacity ) {
	...
	static_assert( sizeof( envelope ) == 48 );
	const auto &header = definitions.header;
	if ( !data || !header.count || header.count > 256 || header.fieldCount > 2048 )
		return 0;
	const size_t bytes = sizeof( header ) + header.count * sizeof( entityDefinition_t ) + header.fieldCount * sizeof( entityDefinitionField_t );
	if ( capacity < sizeof( envelope ) + bytes )
		return 0;
	auto *payload = (uint8_t *)data + sizeof( envelope );
	memcpy( payload, &header, sizeof( header ) );
	memcpy( payload + sizeof( header ), definitions.definitions, header.count * sizeof( entityDefinition_t ) );
	memcpy( payload + sizeof( header ) + header.count * sizeof( entityDefinition_t ), definitions.fields, header.fieldCount * sizeof( entityDefinitionField_t ) );
	envelope.size = uint32_t( bytes );
	calc_sha_256( envelope.hash, payload, bytes );
	memcpy( data, &envelope, sizeof( envelope ) );
	return sizeof( envelope ) + bytes;
}
