...
qboolean R_LoadIQM( model_t *mod, void *buffer, int filesize, const char *mod_name ) {
	...
	LL( header->filesize );
	if( header->filesize > (unsigned int)filesize || header->filesize > (unsigned int)( 16<<20 ) ) {
		return qfalse;
	}
	...
	for ( i = 0; (size_t)i < ARRAY_LEN( vertexArrayFormat ); i++ ) {
	...
	if ( header->num_meshes )
	{
		...
		for( i = 0; (unsigned int)i < header->num_vertexarrays; i++, vertexarray++ ) {
			int	n, *intPtr;
		...
		for( i = 0; (unsigned int)i < header->num_triangles; i++, triangle++ ) {
		...
		for( i = 0; (unsigned int)i < header->num_meshes; i++, mesh++) {
			LL( mesh->name );
			LL( mesh->material );
			...
			if( header->num_joints ) {
				for( j = 0; (unsigned int)j < mesh->num_vertexes; j++ ) {
	...
	if ( header->num_poses )
	{
		...
		for( i = 0; (unsigned int)i < header->num_poses; i++, pose++ ) {
			LL( pose->parent );
			LL( pose->mask );
	...
	if (header->ofs_bounds)
	{
		...
		for(i = 0; (unsigned int)i < header->num_frames; i++)
		{
			LL(bounds->bbmin[0]);
			LL(bounds->bbmin[1]);
			LL(bounds->bbmin[2]);
	...
	if( header->num_meshes )
	{
		...
		for( i = 0; (unsigned int)i < header->num_meshes; i++, mesh++, surface++ ) {
		...
		if( allocateInfluences ) {
			...
			surface = iqmData->surfaces;
			for( i = 0; (unsigned int)i < header->num_meshes; i++, surface++ ) {
	...
	if( header->num_poses )
	{
		...
		for( i = 0; (unsigned int)i < header->num_frames; i++ ) {
			pose = (iqmPose_t *)((byte *)header + header->ofs_poses);
			for( j = 0; (unsigned int)j < header->num_poses; j++, pose++, transform++ ) {
				vec3_t	translate;
				quat_t	rotate;
				vec3_t	scale;
	...
	else if( header->num_meshes && header->num_frames == 0 )
	{
		...
		for ( i = 0 ; (unsigned int)i < header->num_vertexes ; i++ ) {
