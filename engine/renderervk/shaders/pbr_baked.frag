...
// Opt-in q3map2 intensity and model-space direction atlas.
...
layout(set = 4, binding = 0) uniform sampler2D bakedTexture;
...
void main() {
	...
	vec3 geometric = unitOr(objectNormal, vec3(0, 0, 1));
	vec3 n = geometric;
	...
	if (directed.w < 0.5) n = geometric;
	if (!gl_FrontFacing) { n = -n; geometric = -geometric; }
	...
	vec3 baked = texture(bakedTexture, vec2(lightmapUv.x, lightmapUv.y * 0.5)).rgb;
	vec3 l = unitOr(texture(bakedTexture, vec2(lightmapUv.x, lightmapUv.y * 0.5 + 0.5)).rgb * 2.0 - 1.0, geometric);
	vec3 h = unitOr(v + l, n);
	...
	// ponytail: a dominant direction approximates multi-light irradiance; keep the
	// baked diffuse value on flat normals and bound grazing-angle amplification.
	float original = max(dot(geometric, l), 0.0);
	float ratio = directed.w < 0.5 ? 1.0 : clamp(nl / max(original, 0.25), 0.0, 4.0);
	vec3 result = diffuse * baked * ratio + fresnel * distribution * visibility * nl * baked;
	result += em.rgb * emissiveMetallic.rgb;
