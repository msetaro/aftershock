...
static void EvolveFuzzySeperator_r( fuzzyseperator_t *fs ) {
	if ( fs->child ) {
		...
		if ( random() < 0.01 ) {
			double expressionValue = crandom() * ( fs->maxweight - fs->minweight );
			fs->weight = (float)( fs->weight + expressionValue );
		} else {
			double expressionValue = crandom() * ( fs->maxweight - fs->minweight ) * 0.5;
			fs->weight = (float)( fs->weight + expressionValue );
		}
