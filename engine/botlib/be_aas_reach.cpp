...
static float AAS_FaceArea( aas_face_t *face ) {
	...
	for ( i = 1; i < face->numedges - 1; i++ ) {
		...
		{
			double expressionValue = 0.5 * VectorLength( cross );
			total = (float)( total + expressionValue );
		}
	} //end for
	return total;
...
static int AAS_GetJumpPadInfo( int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity ) {
	...
	( ( origin )[0] = (float)( ( origin )[0] * ( 0.5 ) ), ( origin )[1] = (float)( ( origin )[1] * ( 0.5 ) ), ( origin )[2] = (float)( ( origin )[2] * ( 0.5 ) ) );
	...
	time = (float)( sqrt( height / ( 0.5 * gravity ) ) );
	if ( !time ) {
...
static float AAS_FallDelta( float distance ) {
	...
	gravity = aassettings.phys_gravity;
	t = (float)( sqrt( fabs( (double)( distance ) ) * 2 / gravity ) );
	delta = t * gravity;
	return (float)( delta * delta * 0.0001 );
...
static float AAS_MaxJumpHeight( float phys_jumpvel ) {
	...
	return (float)( 0.5 * phys_gravity * ( phys_jumpvel / phys_gravity ) * ( phys_jumpvel / phys_gravity ) );
...
static float AAS_MaxJumpDistance( float phys_jumpvel ) {
	...
	t = (float)( sqrt( aassettings.rs_maxjumpfallheight / ( 0.5 * phys_gravity ) ) );
	//maximum distance
...
static int AAS_Reachability_EqualFloorHeight( int area1num, int area2num ) {
		...
		for ( j = 0; j < area2->numfaces; j++ ) {
			...
			for ( edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++ ) {
				for ( edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++ ) {
					...
					( ( start )[0] = (float)( ( start )[0] * ( 0.5 ) ), ( start )[1] = (float)( ( start )[1] * ( 0.5 ) ), ( start )[2] = (float)( ( start )[2] * ( 0.5 ) ) );
					VectorCopy( start, end );
					...
					( ( start )[0] = (float)( ( start )[0] + ( normal )[0] * ( INSIDEUNITS_WALKSTART ) ), ( start )[1] = (float)( ( start )[1] + ( normal )[1] * ( INSIDEUNITS_WALKSTART ) ), ( start )[2] = (float)( ( start )[2] + ( normal )[2] * ( INSIDEUNITS_WALKSTART ) ) );
					end[2] += 0.125;
					//
	...
	if ( foundreach ) {
		...
		if ( !AAS_AreaCrouch( area1num ) && AAS_AreaCrouch( area2num ) ) {
			{
				float expressionValue = aassettings.rs_startcrouch;
				lreach->traveltime = (unsigned short)( lreach->traveltime + expressionValue );
			}
		} //end if
		/*
		//NOTE: if there's nearby solid or a gap area after this area
...
static int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge( int area1num, int area2num ) {
			...
			for ( j = 0; j < area2->numfaces; j++ ) {
				...
				for ( l = 0; l < groundface2->numedges; l++ ) {
					...
					if ( dist1 > dist2 - 1 && dist1 < dist2 + 1 ) {
						...
						( ( end )[0] = (float)( ( end )[0] * ( 0.5 ) ), ( end )[1] = (float)( ( end )[1] * ( 0.5 ) ), ( end )[2] = (float)( ( end )[2] * ( 0.5 ) ) );
					} //end if
					else if ( dist1 < dist2 ) {
						dist = dist1;
	...
	if ( ground_foundreach ) {
		...
		if ( ground_bestdist >= 0 && ground_bestdist < aassettings.phys_maxstep ) {
			...
			( ( lreach->start )[0] = (float)( ( ground_beststart )[0] + ( ground_bestnormal )[0] * ( INSIDEUNITS_WALKSTART ) ), ( lreach->start )[1] = (float)( ( ground_beststart )[1] + ( ground_bestnormal )[1] * ( INSIDEUNITS_WALKSTART ) ), ( lreach->start )[2] = (float)( ( ground_beststart )[2] + ( ground_bestnormal )[2] * ( INSIDEUNITS_WALKSTART ) ) );
	...
	if ( ground_foundreach ) {
		if ( ground_bestdist < 0 ) {
			...
			if ( !aassettings.rs_maxfallheight || fabs( (double)( ground_bestdist ) ) < aassettings.rs_maxfallheight ) {
				...
				if ( !trace.startsolid && trace.fraction >= 1.0 ) {
					...
					if ( AAS_PointAreaNum( trace.endpos ) == area2num ) {
						...
						if ( i >= numareas ) {
							...
							lreach->traveltime = (unsigned short)( aassettings.rs_startwalkoffledge + fabs( (double)( ground_bestdist ) ) * 50 / aassettings.phys_gravity );
							...
							if ( !AAS_AreaSwim( area2num ) && !AAS_AreaJumpPad( area2num ) ) {
								if ( AAS_FallDelta( ground_bestdist ) > aassettings.phys_falldelta5 ) {
									{
										float expressionValue = aassettings.rs_falldamage5;
										lreach->traveltime = (unsigned short)( lreach->traveltime + expressionValue );
									}
								} //end if
								if ( AAS_FallDelta( ground_bestdist ) > aassettings.phys_falldelta10 ) {
									{
										float expressionValue = aassettings.rs_falldamage10;
										lreach->traveltime = (unsigned short)( lreach->traveltime + expressionValue );
									}
								} //end if
							} //end if
...
static int AAS_Reachability_Jump( int area1num, int area2num ) {
	...
	if ( bestdist > 4 && bestdist < maxjumpdistance ) {
		...
		if ( bestdist <= 48 && fabs( (double)( beststart[2] - bestend[2] ) ) < 8 ) {
			speed = 400;
...
static int AAS_Reachability_Ladder( int area1num, int area2num ) {
	...
	if ( ladderface1 && ladderface2 ) {
		...
		( ( area1point )[0] = (float)( ( area1point )[0] * ( 0.5 ) ), ( area1point )[1] = (float)( ( area1point )[1] * ( 0.5 ) ), ( area1point )[2] = (float)( ( area1point )[2] * ( 0.5 ) ) );
		...
		//
		ladderface1vertical = fabs( (double)( DotProduct( plane1->normal, up ) ) ) < 0.1;
		ladderface2vertical = fabs( (double)( DotProduct( plane2->normal, up ) ) ) < 0.1;
		...
		if ( ladderface1vertical && ladderface2vertical
			 ...
			 && fabs( (double)( DotProduct( sharededgevec, up ) ) ) < 0.7 ) {
			//create a new reachability link
			lreach = AAS_AllocReachability();
			if ( !lreach )
				return qfalse;
		...
		if ( ladderface1vertical ) {
			...
			for ( i = 0; i < area2->numfaces; i++ ) {
				...
				if ( face2->faceflags & FACE_LADDER ) {
					plane2 = &aasworld.planes[face2->planenum];
					if ( fabs( (double)( DotProduct( plane2->normal, up ) ) ) < 0.1 )
						break;
				} //end if
			} //end for
...
static void AAS_Reachability_Elevator( void ) {
	...
	for ( ent = AAS_NextBSPEntity( 0 ); ent; ent = AAS_NextBSPEntity( ent ) ) {
		...
		if ( !strcmp( classname, "func_plat" ) ) {
			...
			( ( platbottom )[0] = (float)( ( pos2 )[0] + ( mids )[0] * ( 0.5 ) ), ( platbottom )[1] = (float)( ( pos2 )[1] + ( mids )[1] * ( 0.5 ) ), ( platbottom )[2] = (float)( ( pos2 )[2] + ( mids )[2] * ( 0.5 ) ) );
			platbottom[2] = maxs[2] - ( pos1[2] - pos2[2] ) + 2;
			//get a point just above the plat in the top position
			VectorAdd( mins, maxs, mids );
			( ( plattop )[0] = (float)( ( pos2 )[0] + ( mids )[0] * ( 0.5 ) ), ( plattop )[1] = (float)( ( pos2 )[1] + ( mids )[1] * ( 0.5 ) ), ( plattop )[2] = (float)( ( pos2 )[2] + ( mids )[2] * ( 0.5 ) ) );
			plattop[2] = maxs[2] + 2;
			//
			/*if (!area1num)
			{
			...
			//
			VectorAdd( mins, maxs, mids );
			( ( mids )[0] = (float)( ( mids )[0] * ( 0.5 ) ), ( mids )[1] = (float)( ( mids )[1] * ( 0.5 ) ), ( mids )[2] = (float)( ( mids )[2] * ( 0.5 ) ) );
			//
			xvals[0] = mins[0];
			xvals[1] = mids[0];
			xvals[2] = maxs[0];
...
//		VectorScale(velocity, speed, velocity);
        ...
        */
		...
		if ( velocity[0] || velocity[1] ) {
			...
			if ( area2num && i < 20 ) {
				for ( link = areas; link; link = link->next_area ) {
					...
					lreach->facenum = (int)( velocity[2] );
					//NOTE: the edgenum is the horizontal velocity
					lreach->edgenum = (int)( sqrt( (double)( velocity[0] * velocity[0] + velocity[1] * velocity[1] ) ) );
		...
		//
		if ( fabs( (double)( velocity[0] ) ) > 100 || fabs( (double)( velocity[1] ) ) > 100 )
			continue;
			...
			for ( i = 0; i < area2->numfaces; i++ ) {
				...
				if ( ret && speed < 150 ) {
					...
					//if (hordist < 1.6 * facecenter[2] - areastart[2])
					{
						...
						//don't enter slime or lava and don't fall from too high
						if ( move.frames < 30 &&
							 !( move.stopevent & ( SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE ) ) && ( move.stopevent & ( SE_HITGROUNDAREA | SE_TOUCHJUMPPAD | SE_TOUCHTELEPORTER ) ) ) {
							...
							if ( !link ) {
								for ( link = areas; link; link = link->next_area ) {
									...
									lreach->facenum = (int)( velocity[2] );
									//NOTE: the edgenum is the horizontal velocity
									lreach->edgenum = (int)( sqrt( (double)( cmdmove[0] * cmdmove[0] + cmdmove[1] * cmdmove[1] ) ) );
									VectorCopy( areastart, lreach->start );
...
static int AAS_Reachability_Grapple( int area1num, int area2num ) {
	...
	for ( i = 0; i < area2->numfaces; i++ ) {
		...
		if ( z / hordist < tan( (double)( 2 * M_PI * mingrappleangle / 360 ) ) )
			continue;
		//
		...
		lreach->traveltime = (unsigned short)( aassettings.rs_startgrapple + VectorLength( dir ) * 0.25 );
		lreach->next = areareachability[area1num];
		areareachability[area1num] = lreach;
		//
		reach_grapple++;
			...
			if ( ret && speed < 300 ) {
				...
				//if (hordist < 1.6 * (facecenter[2] - areastart[2]))
				{
					...
					*/
					...
					//don't enter slime or lava and don't fall from too high
					if ( move.frames < 30 &&
						 !( move.stopevent & ( SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE ) ) && ( move.stopevent & ( SE_HITGROUNDAREA | SE_TOUCHJUMPPAD ) ) ) {
						//create a rocket or bfg jump reachability from area1 to area2
						lreach = AAS_AllocReachability();
						if ( !lreach )
							return qfalse;
			...
			for ( j = 0; j < area->numfaces; j++ ) {
				...
				for ( l = 0; l < face2->numedges; l++ ) {
					...
					if ( abs( edge1num ) == abs( edge2num ) ) {
						...
						if ( aassettings.rs_maxfallheight && fabs( (double)( mid[2] - trace.endpos[2] ) ) > aassettings.rs_maxfallheight )
							break;
						//
						lreach = AAS_AllocReachability();
						if ( !lreach )
						...
						VectorCopy( trace.endpos, lreach->end );
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime = (unsigned short)( aassettings.rs_startwalkoffledge + fabs( (double)( mid[2] - trace.endpos[2] ) ) * 50 / aassettings.phys_gravity );
...
// Changes Globals:		-
//===========================================================================
int AAS_ContinueInitReachability( float time [[maybe_unused]] ) {
