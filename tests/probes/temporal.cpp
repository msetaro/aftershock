...
int main() {
	...
	assert( !R_TemporalReactiveShader( &shader ) );
	shader.numDeforms = 1;
	assert( R_TemporalReactiveShader( &shader ) );
	shader.numDeforms = 0;
	stage.bundle[0].numImageAnimations = 2;
	assert( R_TemporalReactiveShader( &shader ) );
	stage.bundle[0].numImageAnimations = 0;
	stage.bundle[0].isVideoMap = qtrue;
	assert( R_TemporalReactiveShader( &shader ) );
	stage.bundle[0].isVideoMap = qfalse;
	stage.bundle[0].rgbGen = CGEN_WAVEFORM;
	assert( R_TemporalReactiveShader( &shader ) );
	stage.bundle[0].rgbGen = CGEN_IDENTITY;
	texModInfo_t mod{};
	stage.bundle[0].numTexMods = 1;
	stage.bundle[0].texMods = &mod;
	mod.type = TMOD_SCALE;
	assert( !R_TemporalReactiveShader( &shader ) );
	mod.type = TMOD_SCROLL;
	assert( R_TemporalReactiveShader( &shader ) );
	mod.type = TMOD_SCALE;
	shader_t replacement = shader;
	replacement.numDeforms = 1;
	shader.remappedShader = &replacement;
	assert( R_TemporalReactiveShader( &shader ) );
	R_TemporalReset();
	temporalView_t view{};
	view.frame = 10;
