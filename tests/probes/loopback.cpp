...
extern "C" qboolean __wrap__Z17NET_GetLoopPacket8netsrc_tP8netadr_tP5msg_t( netsrc_t sock, netadr_t *from, msg_t *msg )
{
	...
	while ( __real__Z17NET_GetLoopPacket8netsrc_tP8netadr_tP5msg_t( sock, from, msg ) ) {
		...
		if ( Random() % 100 < 5 ) { dropped++; continue; }
		for ( i = 0; i < ARRAY_LEN( packets ); i++ ) if ( !packets[i].used ) break;
		if ( i == ARRAY_LEN( packets ) ) Com_Error( ERR_FATAL, "test loopback queue overflow" );
	...
	for ( i = 0; i < ARRAY_LEN( packets ); i++ ) {
		if ( packets[i].used && packets[i].side == sock && packets[i].due <= now &&
			( selected < 0 || packets[i].due < packets[selected].due ) ) selected = i;
	}
	if ( selected < 0 ) return qfalse;
	if ( packets[selected].length > msg->maxsize ) Com_Error( ERR_FATAL, "test receive buffer too small" );
	if ( packets[selected].serial < last[sock] ) reordered++;
	last[sock] = packets[selected].serial;
	delivered[sock]++;
	memset( from, 0, sizeof( *from ) ); from->type = NA_LOOPBACK;
