...
char *CopyString( const char *s ) {
	char *p = (char *)Z_Malloc( strlen( s ) + 1 );
	strcpy( p, s );
	return p;
}
...
int main( int argc, char **argv ) {
	...
	number( sizeof( usercmd_t ) );
	number( sizeof( entityState_t ) );
	number( sizeof( playerState_t ) );
	number( sizeof( msg_t ) );
	number( sizeof( netadr_t ) );
	number( offsetof( usercmd_t, angles ) );
	number( offsetof( entityState_t, pos ) );
	number( offsetof( playerState_t, origin ) );
	number( offsetof( msg_t, data ) );
	number( sizeof( dheader_t ) );
	number( sizeof( dmodel_t ) );
	number( sizeof( dplane_t ) );
	number( sizeof( dnode_t ) );
	number( sizeof( dleaf_t ) );
	number( sizeof( dbrush_t ) );
	number( sizeof( dbrushside_t ) );
	number( sizeof( drawVert_t ) );
	number( sizeof( dsurface_t ) );
	number( sizeof( md3Header_t ) );
	number( sizeof( md3Surface_t ) );
	number( sizeof( md3Frame_t ) );
	number( sizeof( md3Tag_t ) );
	number( sizeof( md3Triangle_t ) );
	number( sizeof( md3XyzNormal_t ) );
