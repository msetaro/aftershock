...
def main():
    ...
    names = ['diffuse', 'metal', 'rough', 'normal', 'emissive', 'masked', 'blend', 'restored']
    with tempfile.TemporaryDirectory(prefix='aftershock-pbr-') as temporary:
        ...
        for name in names:
            commands += ['screenshot ' + name, 'wait 3', 'echo pbr_' + name, 'wait 100']
        ...
        with log_path.open('wb') as log:
            ...
            try:
                ...
                for name in names:
                    wait_for(lambda: ('pbr_' + name).encode() in log_path.read_bytes(), process)
                    ...
                    if name == 'diffuse':
                        ...
                        Image.new('RGBA', (32, 32), (220, 128, 210, 255)).save(inputs / 'normal.png')
                    elif name == 'normal':
        ...
        assert all(images['diffuse'].getpixel(p) == images['restored'].getpixel(p) for p in points), 'PBR source round trip changed the stable preview'
        (args.output / 'measurements.json').write_text(json.dumps(measurements, indent=2) + '\n')
        print('PASS: owned glTF PBR sphere, metallic/roughness/normal/emission/mask/blend edits and exact round trip')
