...
with tempfile.TemporaryDirectory(prefix='aftershock-reflection-') as temporary:
    ...
    commands += ['quit'];(base/'reflection.cfg').write_text('\n'.join(commands)+'\n')
    ...
    for name in ('off','on','restored'):
        path=base/'screenshots'/(name+'.tga');shutil.copyfile(path,args.output/path.name)
    ...
    print(f'PASS: baked environment changes {changed} dynamic-model channel bytes and restores exactly')
