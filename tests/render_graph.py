...
for name in ('native', 'contract'):
    ...
    if name == 'native':
        ...
        post = run([binary, '--post'], capture_output=True, timeout=10)
        (output / 'native-post.txt').write_bytes(post.stdout)
        print('PASS: 32 native post target/pass/framebuffer/descriptor configurations', flush=True)
        temporal = run([binary, '--temporal'], capture_output=True, timeout=10)
        (output / 'native-temporal.txt').write_bytes(temporal.stdout)
        print('PASS: 16 native temporal configurations retain two distinct persistent history framebuffers', flush=True)
