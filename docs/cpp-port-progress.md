...
Resume at the blocked list only after the authoritative catalog permits the required compatibility transformations or missing target tooling becomes available. Start with q_math.c's float-overload behavior, const-object linkage (huffman_static.c, snd_codec_ogg.c, Vulkan shader arrays), and the native JIT function-pointer conversion. Then complete the remaining const propagation, enum scope, feature-macro and platform blockers listed below, finish JIT T5 boundaries, and rerun the documented gates/runtime checks. Do not redo completed file transformations. Do not begin phase 3 with these blockers.

All independent work available under the current catalog is checkpointed. Native blocked-file attempts were reverted per the stuck rules; Windows inspection casts are retained under the user's explicit no-MinGW exception and remain unverified. No uncatalog engine fix was invented to pass a gate.
...
- Remaining internal JIT T5 targets: vm_x86 ErrJump/BadJump/BadStack/BadOpStack/BadDataRead/BadDataWrite; vm_aarch64 and vm_armv7l OutJump/BadJump/ErrBadProgramStack/ErrBadOpStack/ErrBadDataRead/ErrBadDataWrite; vm_powerpc OutJump/BadJump/ErrBadProgramStack/ErrBadOpStack/debug ErrHighBitsSet. Their translation units cannot pass required catalog compilation/target gates, so no ungated T5 patch was retained. Future fixes must preserve static definitions inside guarded C-linkage blocks. VM_CallCompiled is the host entry calling generated code despite a stale comment; no external-name boundary was identified, so its existing C++ linkage remains.
...
| `code/client/cl_cgame.c` | done | T1: 116, T2: 1, T3: 6; T5: C linkage block for static JIT syscall target and native DLL callback; 1 C object SHA256s unchanged; strict C++/G2/G3 PASS (client/cl_cgame.o, default); G4 advisory FAIL, full diff retained. |
...
| `code/client/cl_ui.c` | done | T1: 80, T2: 1, T3: 8; T5: C linkage block for static JIT syscall target and native DLL callback; 1 C object SHA256s unchanged; strict C++/G2/G3 PASS (client/cl_ui.o, default); G4 advisory FAIL, full diff retained. |
