...
- G5 reproduction: `python3 tools/port/differential_gate.py /tmp/aftershock-cpp-port/differential`; expected exit 1. Exact output committed in tools/port/evidence/g5-differential.log. Twelve groups match: COM_ParseExt, Info, Q_str/va/Com_sprintf, Cmd tokenization, Cvar, FS paths, MSG basic roundtrip, 256 usercmd/entity/playerstate delta roundtrips, adaptive Huffman, numeric/loopback NET_StringToAdr, 1,000 CM_BoxTrace sweeps with both collisions and misses, Q_rsqrt/Q_fabs. Vector math differs C 5c00b4de / C++ 31ee774f, confirming q_math blocker with an identical C driver.
- Collision fixture uses installed user-owned pak0.pk3 maps/q3dm17.bsp, extracted only under /tmp; no game data committed. BSP SHA256 ee1394417b06d92f705088150d7609d6b9f55f5796a9a7626bfe7982e8fc94e8. Reproduction requires that local game data. This replaces bundling a licensed BSP; no external downloads or packages.
