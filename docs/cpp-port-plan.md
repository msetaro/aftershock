...
| clang-tidy | Small `bugprone-*` + `portability-*` subset, changed lines only (`clang-tidy-diff`). No `modernize-*`, no `cppcoreguidelines-*`. | **In force (#8):** `tests/check_tidy.py` enforces duplicate includes/misleading indentation across both renderers. Existing bugprone/portability checks and `performance-*` are advisory; only `modernize-redundant-void-arg` is enabled from modernize, also advisory. `cppcoreguidelines-*` remains cherry-picked, never wholesale. |
