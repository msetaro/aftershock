...
void BotTestAAS( vec3_t origin ) {
	...
	if ( bot_testsolid.integer ) {
		...
		if ( areanum )
			BotAI_Print( PRT_MESSAGE, (char *)"\remtpy area" );
		else
			BotAI_Print( PRT_MESSAGE, (char *)"\r^1SOLID area" );
...
int BotInitLibrary( void ) {
	...
	if ( strlen( buf ) )
		trap_BotLibVarSet( (char *)"basedir", buf );
	//game directory
	trap_Cvar_VariableStringBuffer( "fs_game", buf, sizeof( buf ) );
	if ( strlen( buf ) )
		trap_BotLibVarSet( (char *)"gamedir", buf );
	//cd directory
	trap_Cvar_VariableStringBuffer( "fs_cdpath", buf, sizeof( buf ) );
	if ( strlen( buf ) )
		trap_BotLibVarSet( (char *)"cddir", buf );
	//
#ifdef MISSIONPACK
