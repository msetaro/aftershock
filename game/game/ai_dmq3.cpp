...
int BotGetActivateGoal( bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal ) {
	...
	for ( ent = trap_AAS_NextBSPEntity( 0 ); ent; ent = trap_AAS_NextBSPEntity( ent ) ) {
		if ( !trap_AAS_ValueForBSPEpairKey( ent, (char *)"model", tmpmodel, sizeof( tmpmodel ) ) )
			continue;
	...
	for ( i = 0; i >= 0 && i < 10; ) {
		for ( ent = cur_entities[i]; ent; ent = trap_AAS_NextBSPEntity( ent ) ) {
			if ( !trap_AAS_ValueForBSPEpairKey( ent, (char *)"target", target, sizeof( target ) ) )
				continue;
...
void BotSetupDeathmatchAI( void ) {
	...
	max_bspmodelindex = 0;
	for ( ent = trap_AAS_NextBSPEntity( 0 ); ent; ent = trap_AAS_NextBSPEntity( ent ) ) {
		if ( !trap_AAS_ValueForBSPEpairKey( ent, (char *)"model", model, sizeof( model ) ) )
			continue;
		if ( model[0] == '*' ) {
