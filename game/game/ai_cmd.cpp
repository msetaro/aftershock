...
void BotPrintTeamGoal(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		case LTG_TEAMHELP:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna help a team mate for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_TEAMACCOMPANY:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna accompany a team mate for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_GETFLAG:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna get the flag for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_RUSHBASE:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna rush to the base for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_RETURNFLAG:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna try to return the flag for %1.0f secs\n", netname, t);
			break;
		}
#ifdef MISSIONPACK
		...
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna defend a key area for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_GETITEM:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna get an item for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_KILL:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna kill someone for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_CAMP:
		case LTG_CAMPORDER:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna camp for %1.0f secs\n", netname, t);
			break;
		}
		case LTG_PATROL:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna patrol for %1.0f secs\n", netname, t);
			break;
		}
		default:
		{
			if (bs->ctfroam_time > FloatTime()) {
				...
				BotAI_Print(PRT_MESSAGE, (char *)"%s: I'm gonna roam for %1.0f secs\n", netname, t);
			}
			else {
				BotAI_Print(PRT_MESSAGE, (char *)"%s: I've got a regular goal\n", netname);
			}
		}
	}
}
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match) {
	...
	while(1) {
		if (!trap_BotFindMatch(keyarea, &keyareamatch, MTCONTEXT_PATROLKEYAREA)) {
			trap_EA_SayTeam(bs->client, (char *)"what do you say?");
	...
	if (!newpatrolpoints || !newpatrolpoints->next) {
		trap_EA_SayTeam(bs->client, (char *)"I need more key points to patrol\n");
		BotFreeWaypoints(newpatrolpoints);
		newpatrolpoints = NULL;
		return qfalse;
	}
...
int BotGPSToPosition(char *buf, vec3_t position) {
	...
	for (i = 0; i < 3; i++) {
		...
		BotAI_Print(PRT_MESSAGE, (char *)"%d\n", sign * num);
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	if (client < 0) {
		if (other) BotAI_BotInitialChat(bs, (char *)"whois", teammate, NULL);
		else BotAI_BotInitialChat(bs, (char *)"whois", netname, NULL);
		client = ClientFromName(netname);
	...
	//
	if (bs->teamgoal.entitynum < 0) {
		if (other) BotAI_BotInitialChat(bs, (char *)"whereis", teammate, NULL);
		else BotAI_BotInitialChat(bs, (char *)"whereareyou", netname, NULL);
		client = ClientFromName(netname);
...
void BotMatch_Camp(bot_state_t *bs, bot_match_t *match) {
	...
	if (client < 0) {
		BotAI_BotInitialChat(bs, (char *)"whois", netname, NULL);
		trap_BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
		return;
	}
	//get the match variable
	...
	if (match->subtype & ST_THERE) {
		...
		if (bs->teamgoal.entitynum < 0) {
			BotAI_BotInitialChat(bs, (char *)"whereareyou", netname, NULL);
			client = ClientFromName(netname);
			trap_BotEnterChat(bs->cs, client, CHAT_TELL);
			return;
		}
...
void BotMatch_TaskPreference(bot_state_t *bs, bot_match_t *match) {
	...
	BotAI_BotInitialChat(bs, (char *)"keepinmind", teammatename, NULL);
	trap_BotEnterChat(bs->cs, teammate, CHAT_TELL);
	BotVoiceChatOnly(bs, teammate, (char *)VOICECHAT_YES);
...
void BotMatch_JoinSubteam(bot_state_t *bs, bot_match_t *match) {
	...
	//
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	BotAI_BotInitialChat(bs, (char *)"joinedteam", teammate, NULL);
	client = ClientFromName(netname);
	trap_BotEnterChat(bs->cs, client, CHAT_TELL);
}
...
void BotMatch_LeaveSubteam(bot_state_t *bs, bot_match_t *match) {
	...
	{
		BotAI_BotInitialChat(bs, (char *)"leftteam", bs->subteam, NULL);
		trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
		client = ClientFromName(netname);
		trap_BotEnterChat(bs->cs, client, CHAT_TELL);
	} //end if
...
void BotMatch_WhichTeam(bot_state_t *bs, bot_match_t *match) {
	...
	//
	if (strlen(bs->subteam)) {
		BotAI_BotInitialChat(bs, (char *)"inteam", bs->subteam, NULL);
	}
	else {
		BotAI_BotInitialChat(bs, (char *)"noteam", NULL);
	}
	trap_BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
}
...
void BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match) {
	...
	if (!areanum) {
		if (BotAddressedToBot(bs, match)) {
			BotAI_BotInitialChat(bs, (char *)"checkpoint_invalid", NULL);
			trap_BotEnterChat(bs->cs, client, CHAT_TELL);
		}
		return;
	}
	...
	if (BotAddressedToBot(bs, match)) {
		...
		BotAI_BotInitialChat(bs, (char *)"checkpoint_confirm", cp->name, buf, NULL);
		trap_BotEnterChat(bs->cs, client, CHAT_TELL);
	}
}
...
void BotMatch_Suicide(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//
	trap_EA_Command(bs->client, (char *)"kill");
	//
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = ClientFromName(netname);
	//
	BotVoiceChat(bs, client, (char *)VOICECHAT_TAUNT);
	trap_EA_Action(bs->client, ACTION_AFFIRMATIVE);
}
...
==================
*/
void BotMatch_WhoIsTeamLeader(bot_state_t *bs, bot_match_t *match [[maybe_unused]]) {
	char netname[MAX_MESSAGE_SIZE];
	...
	if (!Q_stricmp(netname, bs->teamleader)) {
		trap_EA_SayTeam(bs->client, (char *)"I'm the team leader\n");
	}
}
...
==================
*/
void BotMatch_WhatIsMyCommand(bot_state_t *bs, bot_match_t *match [[maybe_unused]]) {
	char netname[MAX_NETNAME];
	...
	if (bestitem != -1) {
		if (gametype == GT_CTF
			...
			if (redtt < (redtt + bluett) * 0.4f) {
				...
				BotAI_BotInitialChat(bs, (char *)"location", nearbyitems[bestitem], NULL);
			}
		}
#ifdef MISSIONPACK
...
void BotMatch_Kill(bot_state_t *bs, bot_match_t *match) {
	...
	if (client < 0) {
		BotAI_BotInitialChat(bs, (char *)"whois", enemy, NULL);
		trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
		client = ClientFromName(netname);
		trap_BotEnterChat(bs->cs, client, CHAT_TELL);
		return;
...
void BotMatch_EnterGame(bot_state_t *bs [[maybe_unused]], bot_match_t *match) {
	int client;
	char netname[MAX_NETNAME];
...
int BotMatchMessage(bot_state_t *bs, char *message) {
	...
	switch(match.type)
	{
		...
		{
			trap_EA_SayTeam(bs->client, (char *)"the part of my brain to create formations has been damaged");
			break;
		}
		...
		default:
		{
			BotAI_Print(PRT_MESSAGE, (char *)"unknown match type\n");
			break;
		}
	}
	return qtrue;
